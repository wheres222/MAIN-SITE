 ...
 *
 * reselling.pro was the other provider and no longer exists; variants that
 * only carried its ids now resolve to no provider at all.
 */
import "server-only";
...
export interface DeliveryResult {
  success: boolean;
  message: string;
  /** The provider's delivery/order ID returned on success */
  deliveryId?: string;
...
/**
 * reselling.pro is gone, so the only provider left is Seryx.
 *
 * A variant carrying reselling_product_id or sellauth_id used to route here
 * and now resolves to "none", which makes fulfillVariant report a clear
 * "no delivery provider configured" error instead of spending the request
 * timing out against a host that no longer resolves.
 */
...
export async function fulfillVariant(params: {
  ...
  // customerEmail stays on the signature but is unused: reselling.pro was the
  // only provider that needed it, and Seryx mints keys we deliver ourselves.
  const { variant, quantity, idempotencySeed, variantLabel } = params;
